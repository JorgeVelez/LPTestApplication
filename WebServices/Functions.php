...
function checkAuthentification($username, $password, $gameName, $IP, $session_token)
{
	...
	if($id == "") { end_script('Credenciales incorrectas. 1'); }		// Authentification denied -> Redirect to authentification page
	...
	if(!$passwordIsValid) { end_script('Credenciales incorrectas. 2'); }						// Authentification denied -> Redirect to authentification page
	...
	if(!$accountIsActivated)
	{
		...
		end_script('Usuario no activado.');
	}
...
function register($mail, $username, $password, $IP, $nombreCompleto, $passSimple,  $idGrupo)
{ 
	...
	if(strlen($username)<3) { end_script('Error! usuario corto.'); }
	if(strlen($password)<3) { end_script('Error! contraseña corta.'); }
	
	//---------------------------------: Is account available ? :----------------------------------------
	if(checkMailExists($mail)) { end_script('Usuario ya existe.'); }
	if(checkUsernameExists($username)) { end_script('Error! Nombre existente.'); }
	...
	$query = "INSERT INTO ".$_SESSION['AccountTable']." (id,mail,username,nombre_completo,password,passwords,idGrupo, salt,validation_code,validated,creation_date,last_activity,last_connection_date,current_game) VALUES (NULL,:mail,:username,:nombre_completo,:password,:passwords,:idGrupo,:salt,:validation_code,:validated,NOW(),NOW(),NOW(),:current_game)";
	$parameters = array(':mail' => $mail,
						':username' => $username,
                        ':nombre_completo' => $nombreCompleto,
						':password' => $password,
						':passwords' => $passSimple,
						':idGrupo' => $idGrupo,
						':salt' => $salt,
...
function modify($currentUsername, $mail, $username, $password, $nombreCompleto)
{
	...
	$parameters = array(':mail' => $mail,':username' => 'sdasds',':password' => 'pass',':nombre_completo' => $nombreCompleto,':salt' => 'salt',':id' => $_SESSION['user_id']);
	$stmt = ExecuteQuery($query, $parameters);
	return true;
}
...
function make_thumb($src, $dest, $size) {
	...
	$virtual_image = imagecreatetruecolor($size, $desired_height);
	imagecopyresampled($virtual_image, $source_image, 0, 0, 0, 0, $size, $desired_height, $width, $height);
